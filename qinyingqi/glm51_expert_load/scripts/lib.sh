...
GLM51_EXPERIMENT_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
...
if len(ids) != 8 or len(set(ids)) != 8 or any(value < 0 or value > 7 for value in ids):
    raise SystemExit("HOST_NPU_IDS must contain eight unique IDs from 0..7")
...
container_name_for_run() {
    local run_id="$1"
    sanitize_name "${CONTAINER_PREFIX:-glm51-w4a8-expert}-${run_id}"
}
