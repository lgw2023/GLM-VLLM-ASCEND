...
LABEL_RUN_ID="$(docker inspect "${CONTAINER_NAME}" \
    --format '{{index .Config.Labels "glm51_expert_load.run_id"}}')"
[[ "${LABEL_RUN_ID}" == "${RUN_ID}" ]] || die "ownership label mismatch; refusing to stop"
HOST_IDS="$(docker inspect "${CONTAINER_NAME}" \
    --format '{{index .Config.Labels "glm51_expert_load.host_npu_ids"}}')"
...
if [[ "$(docker inspect "${CONTAINER_NAME}" --format '{{.State.Running}}')" == true ]]; then
    docker stop -t "${STOP_TIMEOUT_SECONDS:-180}" "${CONTAINER_NAME}"
fi
docker inspect "${CONTAINER_NAME}" >"${RUN_DIR}/container.final.inspect.json"
if ((REMOVE == 1)); then
    docker rm "${CONTAINER_NAME}"
fi
printf 'STOP_OK run_id=%s container=%s removed=%s host_npu_ids=%s\n' \
    "${RUN_ID}" "${CONTAINER_NAME}" "${REMOVE}" "${HOST_IDS}"
