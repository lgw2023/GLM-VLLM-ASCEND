...
W8A8_PATCH_MARKER = "# DEEPSEEK_V4_W8A8_ROUTE_CAPTURE_V5"
W8A8_ANCHOR = (
...
CAPTURE_PATCH_MARKER = "# DEEPSEEK_V4_VLLM_TP8_CAPTURE_GATHER_V5"
CAPTURE_ANCHOR = (
...
CAPTURE_REPLACEMENT = (
    "        ctx = get_forward_context()\n"
    "        # DEEPSEEK_V4_VLLM_TP8_CAPTURE_GATHER_V5\n"
    ...
    "            token_num_per_dp = actual_tokens or fallback_tokens or n\n"
    "            sp_ceil = (\n"
    "                (token_num_per_dp + self.tp_size - 1) // self.tp_size\n"
    "                if self.tp_size > 1\n"
    "                else n\n"
    "            )\n"
    "            # Only gather when this rank clearly holds an SP shard. Prefer\n"
    "            # the no-SP path (launch with FLASHCOMM1=0) for expert-load runs.\n"
    "            if (\n"
    "                self.tp_size > 1\n"
    "                and n != token_num_per_dp\n"
    "                and n in {sp_ceil, token_num_per_dp // self.tp_size}\n"
    "            ):\n"
    "                topk_ids = get_tp_group().all_gather(topk_ids, dim=0)\n"
    "                if topk_ids.shape[0] < token_num_per_dp:\n"
    "                    raise AssertionError(\n"
    "                        \"RoutedExpertsCapturer: TP gather is shorter than the \"\n"
    "                        f\"actual token count: gathered={topk_ids.shape[0]}, \"\n"
